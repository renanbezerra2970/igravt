...
export default function GarcomFlow({ establishmentName }: { establishmentName: string }) {
  ...
  const searchCustomer = async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/staff/customers/lookup", {
        method: "POST",
  ...
  const confirmConsumption = async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/staff/consumption", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  return (
    <div className="shell">
      ...
      <div className="scene-head">
        <span className="eyebrow">Fluxo do garçom · {establishmentName}</span>
        <h2>Lançar consumo</h2>
        <p>
          Ligado ao Supabase de verdade e à sua sessão logada: buscar/criar
          cliente pelo WhatsApp e gravar o lançamento cria dados reais que
          aparecem no Painel.
        </p>
      ...
      <div className="stage">
          ...
          <div className="phone-screen">
            {step === 0 && (
              <>
                ...
                <div className="btn-row">
                  <button className="btn btn-primary" disabled={customer === undefined} onClick={() => setStep(1)} style={{ marginLeft: "auto" }}>
                    Continuar
                  </button>
                </div>
              </>
            ...
            {step === 1 && (
              <>
                ...
                <input type="text" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0,00" style={{ fontSize: "1.3rem" }} />
                <div className="card" style={{ marginTop: 14 }}>
        ...
        <div className="sidenote">
          ...
          <dl>
            <div><dt>Sessão</dt><dd>Só chega até aqui quem está logado como equipe de &quot;{establishmentName}&quot;.</dd></div>
            <div><dt>Identificação</dt><dd>Busca real no Supabase, restrita ao seu estabelecimento (RLS).</dd></div>
